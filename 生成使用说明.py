...
C.append(para('这是一个教学平台。你的账号、档案和成绩都以「文件」形式保存在 GitHub 仓库里，每个人一个数据文件。你需要用「邮箱 + 密码 + 学号」注册登录，登录后网页只加载你自己的数据。', bold=True))
C.append(spacer())
...
C.append(table([
    ['项目', '怎么填'],
    ['邮箱', '你自己的邮箱（例如 2026001@stu.example.com）'],
    ['密码', '自己设置，至少 4 位，要记住'],
    ['学号', '老师发的学号（例如 2026001，必须和名单一致）'],
]))
...
C.append(para('2. 点「登录」。登录后右上角会显示你的名字和学号。'))
...
C.append(table([
    ...
    ['提示"学号不存在"', '学号必须和老师发的名单一致，检查有没有输错'],
    ['提示"学号已被注册"', '该学号已被别的邮箱注册，请联系老师'],
...
C.append(para('· 学号：示例学号 2026001 ~ 2026007（对应王小明等 7 人）；在 data/_students.json 中照格式添加即可增加学生。'))
C.append(para('· 数据：每个学生的档案和成绩存在 data/<学号>.json；账号存在 data/_accounts.json。'))
